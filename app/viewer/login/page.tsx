...
const ERROR_MESSAGES: Record<string, string> = {
  not_viewer: "That account isn't registered as a viewer. Ask your admin to add your email under Admin → Viewers.",
};
...
function ViewerLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!viewerRow) {
      ...
      setLoading(false);
      setError(ERROR_MESSAGES.not_viewer);
      return;
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6">
      <div className="card max-w-md w-full">
        ...
        <h1 className="text-xl font-bold mb-1">Viewer login</h1>
        <p className="text-sm text-slate-300 mb-6">Log in to see the students you've been granted access to.</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-sm text-slate-300 mt-4 text-center">
            No account yet?{" "}
            <Link href="/viewer/signup" className="text-brand-400 font-semibold">
              Sign up
            </Link>
          </p>
        </form>
