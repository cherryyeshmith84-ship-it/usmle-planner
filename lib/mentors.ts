 ...
 *  always wins, otherwise it flips from "upcoming" to "completed" the
 *  moment the scheduled end time passes - no added delay, since a mentor
 *  checking their list right at/after the scheduled end expects to see it
 *  move to Completed immediately, not sit in Upcoming a while longer.
 *  Compares actual timestamps (not the raw ISO strings) so this is
 *  correct regardless of exactly how Postgres/PostgREST formats
 *  end_time's timezone suffix or fractional seconds. */
export function getSlotStatus(slot: Pick<MentorSlot, "end_time" | "cancelled_at">): SlotStatus {
  if (slot.cancelled_at) return "cancelled";
  return Date.now() < new Date(slot.end_time).getTime() ? "upcoming" : "completed";
}
